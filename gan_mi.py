...
Original file is located at
    https://colab.research.google.com/drive/1Y9MK8WgmSKhLWPJGKZ9kmf_sQyUrF23s
"""
...
class Generator:
  ...
  deconv_w3 = tf.Variable(glorot_init([3, 3, 1, 64]))
  deconv_b1 = tf.Variable(tf.constant(0.1, shape=[128]), name='gb1')
  deconv_b2 = tf.Variable(tf.constant(0.1, shape=[64]), name='gb2')
  deconv_b3 = tf.Variable(tf.constant(0.1, shape=[1]), name='gb3')
...
class Discriminator:
  ...
  conv_w1 = tf.get_variable('dw1', shape=[3, 3, 1, 64])
  conv_w2 = tf.get_variable('dw2', shape=[4, 4, 64, 128])
  conv_w3 = tf.get_variable('dw3', shape=[3, 3, 128, 128])
  conv_w4 = tf.get_variable('dw4', shape=[4, 4, 128, 256])
  conv_w5 = tf.get_variable('dw5', shape=[3, 3, 256, 256])
  conv_b1 = tf.Variable(tf.constant(0.1, shape=[64]), name='db1') 
  conv_b2 = tf.Variable(tf.constant(0.1, shape=[128]), name='db2') 
  conv_b3 = tf.Variable(tf.constant(0.1, shape=[128]), name='db3') 
  conv_b4 = tf.Variable(tf.constant(0.1, shape=[256]), name='db4') 
  conv_b5 = tf.Variable(tf.constant(0.1, shape=[256]), name='db5') 
  linear_w = tf.Variable(glorot_init([7*7*256, 1]))
  linear_b = tf.Variable(glorot_init([1]))
